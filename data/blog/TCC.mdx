...
In Brazil (where i'm living), when you are in the last year of college or technical courses (1+year), to graduate you need to make the TCC (Trabalho de Conclusão de Curso) which is like the Final Course Project in the college, that will englobes everything you had learned until now, in the Computer Science courses or relateds like mine (Systems Development) in the most of cases you need to make a end-to-end Software, you'll have to apply theoretical knowledge to  solve real-world problems or to conduct a study that adds value to the discipline.

### Our TCC Project

I can't say much but me and my group (4 people) are developing a Study Plataform for college entrance exams. 

Basically the TCC needs to have 3 versions, Web, Desktop and Android.
...
I really don't have big issues with the stack the want us to use, but the Jquery one annoys me a little bit, it's a outdated tech, these days, everyone is using React, Next.JS, Vue.JS, Svelte or Angular.
...
To be honest, i don't know much about the Mobile Version too, but we will use Java or Kotlin on Android Studio (most likely Java) because we learning Java on the Android Studio the whole year instead of Kotlin.
...
But now, When i'm coding the front-end by myself after a long time without touching it, i remember a lot of things from that time, specially the CSS, i could write it using grid and flexbox without much problem and could make it responsible aswell, so i was really happy when i finished.

But i really looking foward to the next year where we'll start the Mobile App and I'll lock myself in Java 200%.

#### Database Modeling
So, before we started to implement the SQL and the physical database, we first had to design the Database Model.

Starting with the ERD (Entity-Relationship Diagram), I had some questions involving cardinality but i could clear my mind after revising my Database class notes.

After that, i did the ERM (Entity-Relationship Model), which was easier to make to be honest, but that's the part i made more mistakes tho 😅.
